...
|--------|---------:|----------:|
| Strategy annual return | **17.56%** | 13.30% |
| Alpha (annualised) | **+4.26%** | 0.00% |
| Sharpe ratio | **0.991** | 0.745 |
| Sortino ratio | **1.600** | 1.289 |
| Max drawdown | -4.63% | **-4.57%** |
| Weekly VaR (95%) | 3.64% | **2.66%** |
| Weekly CVaR (95%) | 3.92% | **3.28%** |
| High-risk stocks excluded | 20% | 0% |
| Illustrative economic gain / year | **$42,555,513** | $0 |
| Illustrative Team ROI | **53.2x** | - |

The strategy delivers **+4.26 percentage points of annualised alpha** over the unfiltered equal-weight benchmark by removing the 20% of stocks the model flags as High risk each week. The benchmark alpha is shown as 0.00% because it is the reference portfolio. The strategy improves the Sharpe ratio from 0.745 to 0.991 and the Sortino ratio from 1.289 to 1.600, while benchmark VaR/CVaR are reported for like-for-like downside-risk comparison.

The maximum drawdown of the strategy (-4.63%) is marginally deeper than the benchmark (-4.57%). This is expected: by concentrating into the remaining 40 stocks, the strategy has slightly higher idiosyncratic concentration risk, which can manifest in drawdown even as the overall return and Sharpe improve.
...
$$\text{Economic Gain} = \text{AUM} \times \alpha = \$1{,}000{,}000{,}000 \times 0.0426 = \mathbf{\$42{,}555{,}513}$$

**Team ROI** (four-person implementation team at a total annual cost of $800,000):

$$\text{Team ROI} = \frac{\text{Economic Gain}}{\text{Team Cost}} = \frac{\$42{,}555{,}513}{\$800{,}000} = \mathbf{53.19\times}$$
...
1. **Illustrative, not audited.** The 41-week evaluation window is the out-of-sample period of the *demo* run. A real implementation would backtest across the full six-year history after a proper walk-forward validation.
